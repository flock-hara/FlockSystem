...
namespace FlockAppC.データ同期
{
    public partial class frmExportReport : Form
    {
        ...
        {
            if (MessageBox.Show("日報データをエクスポートしますか？" + Environment.NewLine + "既に取込まれている日報がある場合は上書きされます。", "確認", MessageBoxButtons.OKCancel) == DialogResult.Cancel) { return; }
            ...
            // エクスポート実行
            var cnt = cls.ExportData(ref this.pgb);

...
namespace FlockAppC.選択画面
{
    public partial class frmSelectStaff : Form
    {
        ...
        public string Select_user_name2 { get; set; }
        public int Select_user_id { get; set; }
        // 対象テーブル
        public string TargetTable { get; set; }
        // 選択キー
        public string Key { get; set; }
        // 担当者選択区分
        public int SelectUserKbn { get; set; }
        ...
        private void SetDgvSelect2()
        {
            ...
            */
            ...
            col10.Name = "i10";
            col10.HeaderText = "i10";
            col10.Width = colWidth;
            col10.Visible = true;

            col11.Name = "i11";
            col11.HeaderText = "i11";
            col11.Width = colWidth;
            col11.Visible = true;
            */
            ...
            /*
            this.dgvSelect2.Columns.Add(col06);
            this.dgvSelect2.Columns.Add(col07);
            this.dgvSelect2.Columns.Add(col08);
            this.dgvSelect2.Columns.Add(col09);
            this.dgvSelect2.Columns.Add(col10);
            this.dgvSelect2.Columns.Add(col11);
            */
            ...
            /*
            this.dgvSelect2.Columns["i6"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;        //セルの文字表示位置
            this.dgvSelect2.Columns["i7"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;        //セルの文字表示位置
            this.dgvSelect2.Columns["i8"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;        //セルの文字表示位置
            this.dgvSelect2.Columns["i9"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;        //セルの文字表示位置
            this.dgvSelect2.Columns["i10"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;       //セルの文字表示位置
            this.dgvSelect2.Columns["i11"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;       //セルの文字表示位置
            */
            ...
            /*
            this.dgvSelect2.Columns["i6"].DefaultCellStyle.Font = new System.Drawing.Font("游ゴシック", 11);             //フォント設定
            this.dgvSelect2.Columns["i7"].DefaultCellStyle.Font = new System.Drawing.Font("游ゴシック", 11);             //フォント設定
            this.dgvSelect2.Columns["i8"].DefaultCellStyle.Font = new System.Drawing.Font("游ゴシック", 11);             //フォント設定
            this.dgvSelect2.Columns["i9"].DefaultCellStyle.Font = new System.Drawing.Font("游ゴシック", 11);             //フォント設定
            this.dgvSelect2.Columns["i10"].DefaultCellStyle.Font = new System.Drawing.Font("游ゴシック", 11);            //フォント設定
            this.dgvSelect2.Columns["i11"].DefaultCellStyle.Font = new System.Drawing.Font("游ゴシック", 11);            //フォント設定
            */
        ...
        private void dgvSelect1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                ...
                using (ClsSqlDb clsSqlDb = new(ClsDbConfig.SQLServerNo))
                {
                    ...
                    {
                        sb.AppendLine("AND");
                        sb.AppendLine("group_id <> 4");
                    }
                    else if (this.rdo2.Checked == true)
                    {
                        sb.AppendLine("AND");
                        sb.AppendLine("group_id = 4");
                    }
                    ...
                    using (DataTable dt_val = clsSqlDb.DMLSelect(sb.ToString()))
                    {
                        if (dt_val.Rows.Count >= 1)
                        {
                            ...
                            foreach (DataRow dr in dt_val.Rows)
                            {
                                ...
                                // 表示アイテムセット（担当者の場合はフルネームをセット）
                                this.dgvSelect2.Rows[rowCnt].Cells[colCnt + 5].Value = dr["id"].ToString();
                                this.dgvSelect2.Rows[rowCnt].Cells[colCnt].Value = dr["fullname"].ToString();           // 表示用
                                ...
                                if (bgcFlg == true)
                                {
                                    ...
                                    //this.dgvSelect2[colCnt, rowCnt].Style.BackColor = Color.Azure;
                                    bgcFlg = false;
                                }
                                else
                                {
        ...
        private void dgvSelect2_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                Select_user_id = int.Parse(this.dgvSelect2.Rows[e.RowIndex].Cells[e.ColumnIndex + 5].Value.ToString());
                Select_user_name = this.dgvSelect2.Rows[e.RowIndex].Cells[e.ColumnIndex + 10].Value.ToString();
            }
            this.Close();
        }
        /// <summary>
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void frmSelectGrid_FormClosing(object sender, FormClosingEventArgs e)
        {
            // MySQL接続解除
            // clsStatic.clsMySQL.DeConnect();  
        }
        /// <summary>
        /// 戻るボタン
        /// </summary>
        ...
        {
            TargetTable = "mst_location";

            // Select1 Disable
            this.dgvSelect1.Enabled = true;
        ...
        private void btnCheck_Click(object sender, EventArgs e)
        {
            ...
            {
                Select_user_id = int.Parse(this.dgvSelect2.Rows[selectedRowIndex].Cells[selectedColumnIndex + 5].Value.ToString());
                Select_user_name = this.dgvSelect2.Rows[selectedRowIndex].Cells[selectedColumnIndex + 10].Value.ToString();
            }
        ...
        private void dgvSelect2_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                ...
                Select_user_id = int.Parse(this.dgvSelect2.Rows[rowIndex].Cells[colIndex + 5].Value.ToString());
                Select_user_name = this.dgvSelect2.Rows[rowIndex].Cells[colIndex + 10].Value.ToString();
        ...
        {
            // 選択区分
            if (SelectUserKbn == 1) { this.rdo1.Checked = true; }
        }
        /// <summary>
        /// あいまい検索ボタン
        /// </summary>
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (this.txtKey.Text == "")
            {
                MessageBox.Show("検索キーワードを入力してください。", "確認", MessageBoxButtons.OK);
                this.txtKey.Focus();
                return;
            }
            // 選択画面表示
            frmSelectItem frm = new()
            {
                Kbn = 5,
                StrKey = this.txtKey.Text
            };
            frm.ShowDialog();
            // 未選択時は処理終了
            if (frm.StrVal == "") { return; }

            // 選択従業員情報セット
            Select_user_id = frm.Value;
            Select_user_name = frm.StrVal;
